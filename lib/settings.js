...
function validateRepo() {
  ...
  if (exists) {
    ...
    if (!/steyep/.test(pushUrl)) {
      sh.exec("git remote set-url origin git@github.com:steyep/alfred-jira.git");
      sh.exec("git remote set-url --push origin " + pushUrl);
    }
  } else {
    return false;
  }
...
function checkForUpdates() {
  ...
  if (cache.get('update', cacheLimit) === undefined && !updates) {
    ...
    let status = sh.exec('git status').toString();
