...
actions.onAction('projects', projects.menu.bind(projects));
...
actions.onAction('mainMenu', query => {
  ...
  if (keychain.find()) {
    ...
    wf.addItems([
      ...
      { title: 'Create issue', valid: false, autocomplete: wf._sep + 'projects ' + wf._sep, icon: 'new2.png'},
...
switch(true) {
  ...
  case /assign/.test(query):
    ...
    break;
  case /projects/.test(query):
    actions.handle('projects', query);
    break;
