...
module.exports = {
  ...
  getTicket: ticket => {
    ...
    let inProgress = Jira.getProgress(ticket.Key) !== false;
    let progress = {
      title: inProgress ? 'Stop Progress' : 'Start Progress',
      arg: inProgress ? 'stopProgress ' + ticket.Key : 'startProgress ' + ticket.Key,
      valid: true,
      subtitle: inProgress ? inProgress : '',
      icon: inProgress ? './resources/icons/stop.png' : './resources/icons/play.png'
    };

    return [
      { title: summary, valid: false, valid: true, arg:'openURL ' + url, icon: './resources/icons/title.png'},
      progress,
      { title: 'Assigned: ' + assignee, valid: false, icon: UserIcon(assignee), 
